...
@pytest.mark.skipif(
    ...
    reason=(
        "TEST_FCM_TOKEN 환경변수가 설정되지 않았습니다. "
        "실제 FCM 전송 테스트를 건너뜁니다."
    ),
)
...
def test_fcm_integration_send_silent_push():
    """
    FCM Silent Push 통합 테스트: 실제 silent push 전송
    ...
    # When: Silent push 전송 (notification 없이 data만)
    test_data = {
